...
-   Besides putting each argument on a different line for readability and storing the results, what changed in the code?
    (names_prefix = "day_") was added.
-   How did this impact how the values are recorded in the `day` column?
    It removed "day_" from each value in the "day" column.
#### Part c {.unnumbered}
...
Generally speaking, the less a person sleeps, the greater their reaction time becomes.
Most subjects experienced an increase in reaction time as the experiment progressed -- what differed was the rate of this increase.
