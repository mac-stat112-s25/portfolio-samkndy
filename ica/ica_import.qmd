...
```{r}
library(tidyverse)
imdb_messy <- read_csv("C:/Users/samth/Documents/Github/comp112/portfolio-samkndy/data/imdb_5000_messy.csv")
```
...
Minimum, 1st quarter, median mean, 3rd quarter, maximum, number of NAs
-   What type of info is provided on categorical variables?
Number of data points for each category, number of NAs
-   What stands out to you in these summaries? Is there anything you'd need to clean before using this data?
The 'color' variable has redundant categories.
...
There are redundant categories. For example, COLOR and color should not be separate categories.
...
B&W: 10, Black and white: 199, color: 30, Color: 4755, COLOR: 30, NA's: 19
...
It would be wrong because we would be tampering with the data set. It is important we preserve the original data so our analysis can be considered valid.
...
imdb_temp <- imdb_messy |>
  ...
  mutate(color = str_replace(color, "color", "Color")) |>
  mutate(color = str_replace(color, "B&W", "Black_White")) |>
  mutate(color = str_replace(color, "Black and White", "Black_White"))
